...
#		print "vtunerConfig : ", self.vtunerConfig
		...
		for vtuner_idx in self.vtunerIndex:
			...
			if vtuner['vtuner_type'] == "satip_client":
				entry = (
				"VIRTUAL TUNER %s" % vtuner_idx,
				"TYPE : %s" % vtuner['vtuner_type'].replace('_',' ').upper(),
				"IP : %s" % vtuner['ipaddr'],
				"TUNER TYPE : %s" % vtuner['tuner_type'],
				"SAT>IP SERVER : %s" % vtuner['desc'],
				vtuner_idx,
				vtuner['tuner_type'],
				vtuner['uuid'],
				)
...
def Plugins(**kwargs):
	pList = []
	pList.append( PluginDescriptor(name=_("SAT>IP Client"), description=_("SAT>IP Client attached to vtuner."), where = PluginDescriptor.WHERE_PLUGINMENU, needsRestart = False, fnc=main) )
	return pList
